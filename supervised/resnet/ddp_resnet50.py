...
val_transforms = tv.transforms.Compose([ tv.transforms.Resize((400, 400), Image.BICUBIC),
                                         tv.transforms.ToTensor(),
                                         tv.transforms.Normalize(means, stds)])
train_dl = DataLoader(CustomDataset(data_path=train_data, transforms=tv_transforms), batch_size=50, num_workers=8, pin_memory=True)
val_dl = DataLoader(CustomDataset(data_path=validation_data, transforms=val_transforms), batch_size = 50, num_workers=8, pin_memory=True)
...
class ResNetTransferLearning(pl.LightningModule):
  def __init__(self, num_classes, model):
    ...
    """self.backbone.fc = nn.Sequential(nn.Linear(self.backbone.fc.in_features, 1024), 
                                          nn.ReLU(), 
                                          nn.Dropout(p=0.5), 
                                          nn.Linear(1024, 512),
                                          nn.ReLU(),
                                          ...
                                          nn.LogSoftmax(dim=1))
     """
    self.backbone.fc = nn.Sequential(nn.Linear(self.backbone.fc.in_features, 256),
                                    nn.ReLU(),
                                    nn.Linear(256, self.num_classes),
                                    nn.LogSoftmax(dim=1))
...
trainer = pl.Trainer(max_epochs=30, gpus=2, flush_logs_every_n_steps = 100, accelerator='ddp', plugins='ddp_sharded', check_val_every_n_epoch=5)
